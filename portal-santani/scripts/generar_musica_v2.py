"""[v2] Pista instrumental original, más luminosa (Sol mayor, 96 BPM). Uso: python generar_musica_v2.py DUR T_IMPACTO

Base: (sin letra) para el Reel: cinematográfica/comercial, 92 BPM.
...
DUR = float(sys.argv[1]) + 1.0
T_IMP = float(sys.argv[2])
FIN = DUR - 4.5  # comienzo del acorde final
N = int(SR * DUR)
rng = np.random.default_rng(3)
OUT = Path(__file__).resolve().parent.parent / 'public/v2/musica_v2.wav'
...
# Progresión (Sol mayor): G – D – Em – C, un acorde por compás (4 tiempos)
ACORDES = [[55, 59, 62], [62, 66, 69], [64, 67, 71], [60, 64, 67]]
BAJOS = [43, 50, 52, 48]
COMPAS = 4 * BEAT
...
add(riser * (t / 1.5) ** 2 * 0.14, T_IMP - 1.5)
n = int(2.5 * SR)
t = np.arange(n) / SR
boom = np.sin(2 * np.pi * np.cumsum(40 + 60 * np.exp(-t * 12)) / SR) * np.exp(-t * 2.2)
add(boom, T_IMP, gain=0.55)

# Acorde final largo (Re mayor)
n = int(5 * SR)
t = np.arange(n) / SR
fin = sum(np.sin(2 * np.pi * midi(x) * t) for x in (43, 55, 59, 62, 67, 71)) * np.exp(-t * 0.7) * np.minimum(1, t / 0.05)
